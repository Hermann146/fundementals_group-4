...
To compare Italy with other countries in total CO2 emissions, we created a line chart showing emissions trends over time. This helped identify whether Italy’s performance was improving or worsening relative to peers.

For Italy’s per-capita CO2 emissions by sector, we used three different visualizations: bar, line, and area charts. These charts allowed a clear view of which sectors contributed most to emissions and how trends changed over the years.

We analyzed Italy’s per-capita emissions from heat and electricity using a line chart, highlighting trends in energy-related emissions and providing insights into the country’s electricity and heat consumption patterns.

We also checked the percentage of Italy’s energy production comes from renewables using a line graph for 1990 to 2022, showing how the share of clean energy has changed over time.

Italy’s energy production in 2022 was represented with a pie chart, showing the proportion of different energy sources and highlighting the share of renewables in the overall energy mix.

Finally, we compared Italy’s environmental policies with other countries in a table, summarizing the main regulations, targets, and initiatives, allowing a quick understanding of Italy’s policy position globally.
...
elec_heat <- co2_sector |> 
  ...
  filter(country %in% c("Italy", "European Union (27)", "France", 
...
-   Italy releases CO2 through heat and electricity less than every other country except France, and US is in peak level, which shows Italy is showing more concern towards usage of carbon electricity.
...
engery_clean <- engery_clean |>
  mutate(
    entity = factor(
      entity,
      level = c("Germany", "United Kingdom","Italy","China","France", "United States" )
    )
  )
...
final <- plot_ly(type='pie', labels=electricty_pd_longer$variables, values=electricty_pd_longer$values, 
               textinfo='percent',
...
-   Italy’s strong dependence on gas keeps the country exposed to price swings and supply risks.

-   Renewable energy is expanding, yet it still falls short of what’s needed to greatly reduce fossil-fuel reliance.

-   The overall mix shows a nation in transition, with public interest focused on cleaner, more secure, and affordable energy.
